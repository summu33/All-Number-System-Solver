...
def Performer():
    global eq
    if typ1.get()=='DECIMAL' and typ3.get()=='MATHEMATICAL EXPRESSIONS':
        eq=str(eval(eq))
        return Fina_Display()
        
    elif (typ1.get()=='HEXADECIMAL' or typ1.get()== 'BINARY' or typ1.get()=='OCTAL') and (typ3.get()=='MATHEMATICAL EXPRESSIONS'):
        return Transformer()
...
def Transformer():
 ...
 #Checking the input type selected by the user by which can convert it according to it by function calling them.
    ...
    oprn.insert(ind,'') if iden=='YES' else 7#inserting the unary identity at teir original place
    print(oprn)
...
def Float_Binary_Calcu(number):
 try:
    b=0
    rp=''
    int_part=int(number);flo_part=num-int_part
...
path='C:/Users/wave/Desktop/Internet Exploral/bg10.jpg' #define the path of the image
...
typ3 = ttk.Combobox(key, textvariable=StringVar(),values=['Select:-','CONVERSIONS','MATHEMATICAL EXPRESSIONS','OTHER FUNCTIONS'],state= 'readonly')
typ3.place(anchor='center',relx=.78,rely=.63,width=175,height=25)
...
b25=Button(key,text='&',font='algerian 15',command=lambda: Expression('&'),bg='sky blue' , borderwidth=5)
